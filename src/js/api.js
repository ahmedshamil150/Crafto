...
export async function getCardProducts() {
  if (!SUPABASE_URL) return [];
  const params = new URLSearchParams({ order: 'sort_order.asc', active: 'eq.true' });
  const res = await fetch(`${SUPABASE_URL}/rest/v1/card_products?${params}`, { headers: headers() });
  if (!res.ok) return [];
  return res.json();
}

export async function getAllCardProducts() {
  const result = await adminFetch({ path: '/rest/v1/card_products?order=sort_order.asc' });
  return result.data || [];
}

export async function createCardProduct(data) {
  const result = await adminFetch({
    path: '/rest/v1/card_products',
    method: 'POST',
    body: data,
    prefer: 'return=representation',
  });
...
export async function updateCardProduct(id, data) {
  const result = await adminFetch({
    path: `/rest/v1/card_products?id=eq.${id}`,
    method: 'PATCH',
    body: data,
    prefer: 'return=representation',
  });
